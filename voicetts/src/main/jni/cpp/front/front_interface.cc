...
int FrontEngineInterface::init() {
    ...
    g2pmodel = new G2pEModel(_conf_file);

    
    // 生成词典（词到音素的映射）
    if (0 != GenDict(_word2phone_path, word_phone_map,true)) {
        std::cout << "Genarate word2phone dict failed" << std::endl;
        return -1;
    }

    // 生成音素字典（音素到音素id的映射）
    if (0 != GenDict(_phone2id_path, phone_id_map,false)) {
        std::cout << "Genarate phone2id dict failed" << std::endl;
        return -1;
    }

    // 生成音调字典（音调到音调id的映射）
    if (_seperate_tone == "true") {
        if (0 != GenDict(_tone2id_path, tone_id_map,false)) {
            std::cout << "Genarate tone2id dict failed" << std::endl;
            return -1;
        }            
    }

    // 生成繁简字典（繁体到简体id的映射）
    if (0 != GenDict(_trand2simp_path, trand_simp_map, true)) {
        std::cout << "Genarate trand2simp dict failed" << std::endl;
        return -1;
    }

    // 发声校验单词
    if (0 != GenTDict(_orderdict_path,orderdict_map)) {
        std::cout << "Genarate cmu_map dict failed" << std::endl;
        return -1;
    }

    // 单词cmu发声
    if (0 != GenTDict(_cmu_path, cmu_map)) {
        std::cout << "Genarate cmu_map dict failed" << std::endl;
        return -1;
    }
    ...
    if (0 != GenTDict(_homograph2features_path, homograph2features_map)) {
        std::cout << "Genarate cmu_map dict failed" << std::endl;
        return -1;
    }
...
int FrontEngineInterface::ReadConfFile() {
    ...
    std::string path=_conf_file.c_str();

    std::string pathfile=path+"/front.conf";

    if (!speechnn::isFileExist(pathfile)) {
        std::cout << "configuration file " << _conf_file << " does not exists. " << std::endl;
        return -1;
    }
    std::ifstream is(pathfile, std::ifstream::in);
    if (!is.good()) {
        std::cout << "Cannot open config file: " << _conf_file << std::endl;
        return -1;
    }
...
int FrontEngineInterface::GenDict(const std::string &file, std::map<std::string, std::string> &map,bool flag) {
    if (!speechnn::isFileExist(file.c_str())) {
        std::cout  << "configuration file " << file << " does not exists. " << std::endl;
        return -1;
    }
    std::ifstream is(file.c_str(), std::ifstream::in);
    if (!is.good()) {
        std::cout << "Cannot open config file: " << file << std::endl;
        return -1;
    }
    std::string line, key, value;
    while (std::getline(is, line)) {
...
int FrontEngineInterface::GenTDict(const std::string& file, std::map<std::string, std::string>& map) {
    if (!speechnn::isFileExist(file.c_str())) {
        std::cout << "configuration file " << file << " does not exists. " << std::endl;
        return -1;
    }
...
int FrontEngineInterface::GetSentenceIds(const std::string &sentence, std::vector<int> &phoneids, std::vector<int> &toneids) {
    ...
    if (0 != Cut(sentence, cut_result)) {
        std::cout << "Cut sentence: \"" << sentence << "\" failed" << std::endl;
        return -1;
    }
    
    if (0 != GetWordsIds(cut_result, phoneids, toneids)) {
        std::cout << "Get words phoneids failed" << std::endl;
        return -1;
    }
    return 0;
}
    ...
    for(int i = 0; i < cut_result.size(); i++) {
            ...
            if (std::find(_punc_omit.begin(), _punc_omit.end(), word) == _punc_omit.end()) { // 非可忽略的标点
                ...
                if (std::find(_punc.begin(), _punc.end(), word) == _punc.end()) { // 文字
                    ...
                    if (0 != GetInitialsFinals(word, word_initials, word_finals)) {
                        std::cout << "Genarate the word_initials and word_finals of " << word << " failed" << std::endl;
                        return -1;
                    }

                    // 对读音进行修改
                    if (0 != ModifyTone(word, pos, word_finals)) {
                        std::cout << "Failed to modify tone." << std::endl;
                    }
                ...
                if (0 != Phone2Phoneid(phone, phoneids, toneids)) {
                    std::cout << "Genarate the phone id of " << word << " failed"<< std::endl;
                    return -1;
                }
            }
        }
        else
        {
            ...
            //{
                ...
                /*
                if (homograph2features_map[word].size() > 0)
                {
                    std::vector<std::string> word_one;
                    Stringsplit(homograph2features_map[word], '|', word_one);
                    Stringsplit(word_one[0], ',', word_final);
                }
                */
                if (cmu_map[word].size() > 0)
                {
                    std::vector<std::string> word_one;
                    Stringsplit(cmu_map[word], '|', word_one);
                    Stringsplit(word_one[0], ',', word_final);
                }
                else
                {
                    std::vector<const char*> word_char=g2pmodel->g2p(word);
                    for (size_t i = 0; i < word_char.size(); i++)
                    {
                        word_final.push_back(word_char[i]);
                    }
                }
...
bool IsCheckLetter(const std::string& str)
{
    int m = 0;
...
int FrontEngineInterface::Cut(const std::string &sentence, std::vector<std::pair<std::string, std::string>> &cut_result) {
    ...
    {
        if (IsCheckLetter(cut_result_jieba[i].first)&& cut_result_jieba[i].second != "eng") {
    ...
    if (0 != MergeforModify(cut_result_jieba, cut_result)) {
        std::cout << "Failed to modify  for word segmentation result."<< std::endl;
        return -1;
    }
...
//    if(word_finals.size() == speechnn::utf8string2wstring(word).length() && word_finals.size() == word_initials.size())
//    {
//        std::cout << "Failed to cuts word finals"<<std::endl;
//        return -1;
//    }
...
int FrontEngineInterface::GetFinals(const std::string &word, std::vector<std::string> &word_finals) {
    std::vector<std::string> word_initials;
    if(0 != GetInitialsFinals(word, word_initials, word_finals)) {
        std::cout << "Failed to get word finals"<<std::endl;
        return -1;
    }
        ...
        if ("x" == pos&&word!=" ")
        {
            if (std::find(_punc_omit.begin(), _punc_omit.end(), word) == _punc_omit.end()) { // 非可忽略的标点，即文字
                if (0 != GetFinals(word, word_final)) {
                    std::cout << "Failed to get the final of word."<< std::endl;
                }
            }
        }
...
std::vector<std::pair<std::string, std::string>> FrontEngineInterface::MergeThreeTones2(std::vector<std::pair<std::string, std::string>> &seg_result) {
    ...
    for(int i = 0; i < word_num; i++) {
        ...
        if ("x" == pos && word != " ")
        {
            if (i - 1 >= 0 && !finals[i - 1].empty() && finals[i - 1].back().back()=='3' &&
                !finals[i].empty() && finals[i].front().back()=='3' && !merge_last[i - 1]) {
                // if the last word is reduplication, not merge, because reduplication need to be _neural_sandhi
                if (!IsReduplication(seg_result[i - 1].first) &&
                    (speechnn::utf8string2wstring(seg_result[i - 1].first)).length() + speechnn::utf8string2wstring(word).length() <= 3) {
                    result.back().first = result.back().first + seg_result[i].first;
...
int FrontEngineInterface::MergeforModify(std::vector<std::pair<std::string, std::string>> &seg_word_type, 
    ...
    std::cout << "Before merge, seg result is: " << limonp::Join(seg_result.begin(), seg_result.end(), "/")<< std::endl;
    ...
    std::cout << "After merge, seg result is: " << limonp::Join(seg_result.begin(), seg_result.end(), "/")<< std::endl;
...
int FrontEngineInterface::BuSandi(const std::string &word, std::vector<std::string> &finals) {
    ...
    if(0 != Word2WordVec(word, wordvec)) {
        std::cout << "Failed to get word vector"<< std::endl;
        return -1;
    }
    ...
    if(wordvec.size() == 3 && wordvec[1] == bu) {
        ...
        for(int i = 0; i < wordvec.size(); i++) {
            if(wordvec[i] == bu && i + 1 < wordvec.size() && finals[i + 1].back()== '4') {
...
int FrontEngineInterface::NeuralSandhi(const std::string &word, const std::string &pos, std::vector<std::string> &finals) {
    ...
    if(0 != SplitWord(word, word_list)) {
        std::cout << "Failed to split word."<< std::endl;
        return -1;
    }
    if (finals.size()==0)
    {
    ...
    if(word_num == 2 && AllToneThree(finals)) {
        ...
        if(AllToneThree(finals)) {
            ...
            for(int i = 0; i < finals_list.size(); i++) {
                ...
                } else if(i == 1 && !(AllToneThree(finals_list[i])) && finals_list[i][0].back() == '3'&& finals_list[0].back().back() == '3'){
...
int FrontEngineInterface::ModifyTone(const std::string &word, const std::string &pos, std::vector<std::string> &finals) {
    if((0 != BuSandi(word, finals)) || (0 != YiSandhi(word, finals)) ||    
       (0 != NeuralSandhi(word, pos, finals)) || (0 != ThreeSandhi(word,finals))) {   
           std::cout << "Failed to modify tone of the word: " << word<< std::endl;
           return -1;
       } 
...
void FrontEngineInterface::ToLower(std::string &s) {
    int len=s.size();
    for(int i=0;i<len;i++){
